...
def test_enhanced_patterns():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if pattern_accuracy >= 50:  # At least half the expected patterns found
            pattern_matches += 1
            print(f"   ✅ Pattern Recognition: {pattern_accuracy:.0f}% ({', '.join(detected_patterns)})")
        else:
            print(f"   ❌ Pattern Recognition: {pattern_accuracy:.0f}% (expected better match)")
        ...
        if predicted_workflow == test_case['workflow_type']:
            ...
            print(f"   🎯 Workflow Prediction: ❌ Got {predicted_workflow}, expected {test_case['workflow_type']}")
        ...
        if has_priority:
            ...
            print(f"   🚨 Priority Detection: ℹ️  Normal priority")
    ...
    if overall_score >= 90:
        ...
        print("   ⚠️  NEEDS WORK: Enhanced pattern integration needs improvement")
